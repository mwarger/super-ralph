...
**[Superpowers](https://github.com/obra/superpowers)** is a skills framework and SDLC methodology for AI coding agents. It provides mandatory, composable skills that govern how an agent thinks and works: brainstorming, planning, TDD, code review, debugging, and branch management. Skills trigger automatically and are not optional. The `brainstorming` and `writing-plans` skills are the core of the intake process. They are not replaced — they are invoked, extended, and their principles heavily reused.

**[Ralph TUI](https://github.com/subsy/ralph-tui)** is a terminal UI that orchestrates AI coding agents through autonomous loops. It connects an agent to a task tracker and runs a SELECT -> PROMPT -> EXECUTE -> EVALUATE cycle, completing tasks one by one with session persistence, error handling, and real-time visibility. It implements Geoffrey Huntley's "[Ralph Wiggum method](https://ghuntley.com/ralph)" — a monolithic, single-process approach where each iteration gets a fresh context window and does one thing. Its key extension points: `--prd-skill` for custom intake, custom Handlebars prompt templates, and `skills_dir` for custom bead conversion.

**[Emanuel's Flywheel](https://github.com/jeffreyemanuel)** is a collection of 14 interconnected tools for multi-agent development. The relevant pieces for this framework are: [Beads Viewer (BV)](https://github.com/jeffreyemanuel/beads-viewer) for graph-theoretic task prioritization using PageRank, [CASS](https://github.com/jeffreyemanuel/cass) for indexing and searching past agent sessions, the CASS Memory System for persistent cross-session learning, and Emanuel's iterative plan refinement methodology. Emanuel's workflow also provides battle-tested prompt patterns for code review, bug scanning, and UI/UX polish that become bead types in this framework.
